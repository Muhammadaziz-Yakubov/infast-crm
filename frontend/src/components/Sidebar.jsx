...
const adminMenu = [
    ...
    { path: '/challenges', label: 'Chellenjlar', icon: HiOutlineFire },
...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 left-0 h-full w-72 bg-white dark:bg-dark-900 border-r border-gray-200 dark:border-white/5 z-50 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                <div className="p-8 pb-10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <h1 className="text-xl font-black text-gray-900 dark:text-white leading-none tracking-tighter">In<span className="text-primary-500">Fast</span></h1>
                ...
                <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
                    {menuItems.map((item) => (
                        <NavLink key={item.path} to={item.path} onClick={onClose}
                            className={({ isActive }) => `flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all font-bold text-sm ${isActive ? 'bg-primary-500 text-white shadow-lg' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-dark-800'}`}>
                ...
                <div className="p-6 border-t border-gray-100 dark:border-white/5">
                    <button onClick={toggleDarkMode} className="w-full flex items-center justify-between p-3 rounded-2xl bg-gray-50 dark:bg-dark-800 mb-4">
                    ...
                    {user ? (
                        ...
                        <NavLink to="/login" className="flex items-center justify-center gap-3 w-full py-3.5 rounded-2xl bg-primary-500 text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-primary-500/20 active:scale-95 transition-all">
