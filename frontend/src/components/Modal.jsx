...
const Modal = ({ isOpen, onClose, title, children, size = 'md' }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] overflow-y-auto">
            {/* Overlay */}
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm modal-overlay" onClick={onClose} />

            {/* Modal Container */}
            <div className="flex min-h-full items-center justify-center p-4">
                {/* Modal Content */}
                <div className={`relative w-full ${sizes[size]} bg-white dark:bg-dark-800 rounded-[2.5rem] 
                    shadow-2xl modal-content flex flex-col border border-white/10 overflow-hidden`}>
                    {/* Header */}
                    ...
                    {/* Body */}
                    <div className="px-6 py-5 flex-1 overflow-y-visible">
                        {children}
                    </div>
                </div>
            </div>
